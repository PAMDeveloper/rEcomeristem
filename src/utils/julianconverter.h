#ifndef JULIAN_DATE_TIME_HPP
#define JULIAN_DATE_TIME_HPP
...
enum JulianDateTimeUnitOptions
{
    DATE_TIME_UNIT_NONE,
    DATE_TIME_UNIT_DAY,
    DATE_TIME_UNIT_WEEK,
...
struct _date
{
   ...
   static long intern_aMonth(long year, long month) noexcept
    ...
    _date(long year, long month, long day, double partofday) noexcept
    : myear(year)
    , mmonth(month)
    , mday(day)
    , mhours(0)
    ...
    _date(const _date& d) = default;
    _date& operator=(const _date& d) = default;
    _date(_date&& d) = default;
    _date& operator=(_date&& d) = default;
    ...
    std::string toString(JulianDateFormat fmt) noexcept
    {
    std::stringstream ss;
    ...
    bool fromString(const std::string& date, JulianDateFormat toparse) noexcept
    {
    bool error = false;
    ...
    if (toparse == DATE_FORMAT_EXTENDED) {
        ...
        while (next != end) {
            ...
            if (nbmatches == 1) {
              ...
              } else if (toparse == DATE_FORMAT_DMY) {
    ...
    bool equals(const _date& d) const noexcept
    {
    return (myear == d.myear and mmonth == d.mmonth and mday == d.mday);
    }

    bool inf(const _date& d) const noexcept
    {
    if (myear < d.myear)
        return true;
    ...
    bool sup(const _date& d) const noexcept
    {
    if (myear < d.myear)
        return false;
    ...
    {
    unsigned long julianbegin = _date(myear, 1, 1, 0).julianDay();
    ...
    //year as the beginning year
        ...
        julianbegin = _date(myear-1, 1, 1, 0).julianDay();
        day = (julianbegin + 3) % 7;
    ...
    long daysUntil(const _date& d) const noexcept
    {
    if (equals(d))
        return 0;
...
//                            long& day);
    ...
    static bool isLeapYear(double time)
    {
        _date d;
        d.fromJulianDay(time);
        return _date::intern_isLeapYear(d.myear);;
    }
    ...
    static double aMonth(double time)
    {
        _date d;
        d.fromJulianDay(time);
        return(_date::intern_aMonth(d.myear, d.mmonth));
    ...
    static JulianDateTimeUnitOptions convertUnit(const std::string& unit)
    {
        if (unit == "day") {
    ...
    static double duration(double time, double duration,
        JulianDateTimeUnitOptions unit)
    {
        switch (unit) {
  ...
  static long toJulianDayNumber(const std::string& date, JulianDateFormat format)
  {
    _date d;
    d.fromString(date, format);
    return d.julianDayNumber();
  }
    ...
    static std::string toJulianDayFmt(double date, JulianDateFormat format){
        _date d;
        d.fromJulianDay(date);
  ...
  static double toJulianDay(const std::string& date, JulianDateFormat format)
  {
    _date d;
    d.fromString(date, format);
    return d.julianDay();
